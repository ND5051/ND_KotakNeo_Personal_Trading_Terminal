import os
import os
import sys
import getpass

# Add the SDK directory to sys.path so we can import neo_api_client directly
sdk_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "kotak-neo-python")
if os.path.exists(sdk_path) and sdk_path not in sys.path:
    sys.path.insert(0, sdk_path)

# Try importing pyotp to generate TOTP code if secret is provided
try:
    import pyotp
except ImportError:
    pyotp = None

# Simple dotenv parser to load .env variables if .env file exists in the current directory
def load_dotenv():
    env_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".env")
    if os.path.exists(env_path):
        with open(env_path, "r") as f:
            for line in f:
                line = line.strip()
                if not line or line.startswith("#"):
                    continue
                if "=" in line:
                    key, val = line.split("=", 1)
                    key = key.strip()
                    val = val.strip().strip('"').strip("'")
                    if key and val:
                        os.environ[key] = val

# Load configuration from environment/dotenv if present
load_dotenv()

def get_input(prompt_text, env_var, is_secret=False):
    default_val = os.environ.get(env_var, "")
    prompt_suffix = f" [{default_val[:4]}...{default_val[-4:]}]" if default_val and len(default_val) > 8 else (f" [{default_val}]" if default_val else "")
    
    if is_secret:
        if default_val:
            val = getpass.getpass(f"{prompt_text}{prompt_suffix} (Press Enter to use default): ")
            return val if val else default_val
        else:
            return getpass.getpass(f"{prompt_text}: ")
    else:
        val = input(f"{prompt_text}{prompt_suffix}: ")
        return val if val else default_val

def main():
    print("=========================================")
    print("  Kotak Neo Interactive Login Setup")
    print("=========================================")

    # Prompt user for all required credentials
    consumer_key = get_input("1. Enter Consumer Key (from Trade API card)", "NEO_CONSUMER_KEY")
    mobile_number = get_input("2. Enter Mobile Number (e.g. +91XXXXXXXXXX)", "NEO_MOBILE_NUMBER")
    ucc = get_input("3. Enter UCC (Unique Client Code)", "NEO_UCC")
    mpin = get_input("4. Enter MPIN (6-digit PIN)", "NEO_MPIN", is_secret=True)

    # TOTP is asked last in the sequence:
    totp_code = ""
    totp_secret = os.environ.get("NEO_TOTP_SECRET", "")

    # We ask for the live TOTP code or secret key
    totp_input = input("5. Enter live 6-digit TOTP code (or press Enter to use/input a TOTP Secret Key): ").strip()

    if not totp_input:
        # They want to use/enter a secret key
        if not totp_secret:
            totp_secret = getpass.getpass("Enter TOTP Secret Key (for auto-generation): ").strip()
        
        if totp_secret:
            if pyotp is None:
                print("\nError: 'pyotp' package is not installed. Cannot auto-generate TOTP.")
                print("Please run: pip install pyotp")
                sys.exit(1)
            try:
                totp_code = pyotp.TOTP(totp_secret).now()
                print(f"Generated TOTP Code: {totp_code}")
            except Exception as e:
                print(f"\nError generating TOTP: {e}")
                sys.exit(1)
        else:
            print("\nError: No TOTP code or TOTP Secret Key provided.")
            sys.exit(1)
    else:
        totp_code = totp_input

    # Validate inputs before sending
    if not (consumer_key and mobile_number and ucc and mpin and totp_code):
        print("\nError: All fields are required to proceed with login.")
        sys.exit(1)

    print("\nInitializing NeoAPI client...")
    from neo_api_client import NeoAPI

    client = NeoAPI(
        consumer_key=consumer_key,
        environment="prod"
    )

    try:
        print("Sending TOTP login request...")
        login_response = client.totp_login(
            mobile_number=mobile_number,
            ucc=ucc,
            totp=totp_code
        )
        print(f"Login Response: {login_response}")
        
        # Check success by verifying that 'data' and 'token' are returned in the response
        if login_response.get("data") and "token" in login_response["data"]:
            print("TOTP validation successful. Now validating MPIN...")
            validate_response = client.totp_validate(mpin=mpin)
            print(f"Validate Response: {validate_response}")
            
            if validate_response.get("data") and "token" in validate_response["data"]:
                print("\n✓ Login and session validation successful! You are now logged in.")
            else:
                print("\n✗ MPIN validation failed.")
        else:
            print("\n✗ TOTP login failed.")
    except Exception as e:
        print(f"\n✗ Error during login process: {e}")

if __name__ == "__main__":
    main()
